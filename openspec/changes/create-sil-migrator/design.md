...
- Must remain independent (zero required dependencies on toutago-datamapper or other ORMs)
- Should provide optional datamapper integration through adapter pattern
...
- Provide optional integration with toutago-datamapper for users who already use it
...
- Maintain zero required dependencies (toutago-datamapper integration is opt-in)
...
- MySQL adapter
- SQLite adapter
- **Datamapper bridge adapter** (optional integration with toutago-datamapper)
...
### Decision 8: Optional Toutago-Datamapper Integration
**What**: Provide a bridge adapter that allows Síl to use toutago-datamapper's database adapters.

**Why**:
- Users already using datamapper can reuse their configuration and adapters
- Leverages datamapper's production-ready MySQL and PostgreSQL adapters
- Enables consistent database connection management across tools
- Maintains independence - datamapper integration is completely optional

**Implementation**:
```go
// DatamapperAdapter wraps a datamapper adapter for Síl migrations
type DatamapperAdapter struct {
    mapper *engine.Mapper
    source string
}

// Example usage with datamapper:
mapper, _ := engine.NewMapper("config.yaml")
silAdapter := sil.NewDatamapperBridge(mapper, "main-db")
migrator := sil.NewMigrator(sil.Config{
    Adapter: silAdapter,
    MigrationsDir: "./migrations",
})

// Example usage without datamapper (standalone):
postgresAdapter := sil.NewPostgresAdapter("postgres://user:pass@localhost/db")
migrator := sil.NewMigrator(sil.Config{
...
**Benefits**:
- ✅ Users with datamapper get seamless integration
- ✅ Users without datamapper can still use Síl standalone
- ✅ Configuration can be shared between datamapper and Síl
- ✅ No required dependency - import only if needed
- ✅ Same interface for migrations regardless of adapter choice

**Alternatives considered**:
- **Required datamapper dependency**: Would limit Síl's standalone use
- **No datamapper support**: Would force users to duplicate database configuration
- **Bridge as separate package**: Adds complexity for simple integration
...
│       │   ├── datamapper.go   # Datamapper bridge adapter (optional)
│       │   └── mock.go         # Testing adapter
│       └── templates/          # Migration templates
├── examples/
│   ├── basic/                  # Basic usage example
│   ├── multi-db/               # Multi-database example
│   ├── with-datamapper/        # Datamapper integration example
...
- [ ] MySQL adapter
- [ ] SQLite adapter
- [ ] Datamapper bridge adapter (optional integration)
- [ ] Multi-database testing
- [ ] Adapter documentation
- [ ] Datamapper integration example
