...
- **Zero required dependencies**: Can be used as standalone library without toutago-datamapper
- **Optional datamapper integration**: Provide a bridge adapter to use datamapper's adapters for database operations

### Phased Implementation
- **Phase 1**: Core migration engine and PostgreSQL adapter
- **Phase 2**: Multi-database support (MySQL, SQLite) and datamapper bridge adapter
...
- **Optional toutago-datamapper integration**:
  - Bridge adapter allows using datamapper's database adapters (MySQL, PostgreSQL, etc.)
  - Migrations can leverage datamapper configuration for connection management
  - No required dependency - datamapper integration is opt-in
  - Users without datamapper can still use Síl with built-in database adapters
...
- Phase 2: 2-3 weeks (multi-database support + datamapper integration)
